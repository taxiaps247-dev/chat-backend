...
def chats(token: str):
    try:
        ...
        return {"items": get_user_chats(uid)}
...
def chat_messages(chat_id: str, token: str, limit: int = 50):
    try:
        ...
        return {"items": get_chat_messages(chat_id, limit=limit)}
...
async def delete_chat(chat_id: str, token: str):
    try:
        decoded = verify_firebase_token(token)
        uid = decoded["uid"]
        ...
        if not profile or profile.get("collection") != "UsersAdministration":
            raise HTTPException(
                status_code=403,
                detail="Solo administrador puede cerrar conversaciones",
            )
        ...
        if not chat_doc.exists:
            return {
                ...
                "chatId": chat_id,
                "participants": [],
                "message": "La conversación ya no existe",
            }
        ...
        # Avisar a participantes conectados antes de borrar.
        payload = {
            "type": "conversation_closed",
            "chatId": chat_id,
            "closedBy": uid,
            "message": "La conversación fue cerrada por administración.",
        }

        for participant_uid in participants:
            await manager.send_to_user(participant_uid, payload)
